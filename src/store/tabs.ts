...
const initialState = {
	// 打开的标签页
	openTabs: new Map<string, TabItemProps>([]),
	// 当前激活的标签页
	activeKey: "",
	// 标签页是否处于刷新状态
	isRefresh: false,
...
interface TabsAction {
	...
	addTab: (routePath: string, tabProps: TabItemProps) => void
	insertBeforeTab: (routePath: string, tabProps: TabItemProps) => void
 ...
 * 标签页状态管理
 */
export const useTabsStore = create<TabsState & TabsAction>(set => ({
	...initialState,
	 ...
	 * 在最前面插入标签页
	 */
	insertBeforeTab: (routePath: string, tabProps: TabItemProps) => {
		set((state) => {
			if (routePath.length) {
	 ...
	 * 添加标签页
	 */
	addTab: (routePath: string, tabProps: TabItemProps) => {
		set((state) => {
			if (routePath.length) {
